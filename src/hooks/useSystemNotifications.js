...
import { getNotificationPermission, requestNotificationPermission, showEventReminder, } from "../utils/systemNotificationUtils";
 ...
 */
export const useSystemNotifications = (events) => {
    ...
    useEffect(() => {
        ...
        const checkUpcomingEvents = () => {
            ...
            events.forEach((event) => {
                if (event.date !== today)
                    return; // Only check today's events
                const [hours, minutes] = event.startTime.split(":").map(Number);
                const eventTime = new Date();
                eventTime.setHours(hours, minutes, 0, 0);
                const minutesUntilEvent = Math.round((eventTime.getTime() - now.getTime()) / 60000);
