...
export const useStatistics = (events) => {
    return useMemo(() => {
        ...
        // Completion rate
        const completionRatePercent = totalEventsToday > 0
        ...
        const averageEventsPerDay = uniqueDates.size > 0 ? Math.round(events.length / uniqueDates.size) : 0;
        // Most used color
        const colorCounts = {};
        events.forEach((e) => {
            colorCounts[e.color] = (colorCounts[e.color] || 0) + 1;
        });
        const mostUsedColorEntry = Object.entries(colorCounts).sort((a, b) => b[1] - a[1])[0];
        ...
        const averageEventDuration = eventCount > 0 ? Math.round(totalDuration / eventCount) : 0;
